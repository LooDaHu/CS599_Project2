...
def train(dataset: str) -> float:
    ...
    for epoch in range(FLAGS.epochs):
        ...
        # Validation
        cost, acc, pred, labels, duration = evaluate(sess, model,
    ...
    # Testing
    test_cost, test_acc, pred, labels, test_duration = evaluate(sess, model,
    ...
    f = open('./text_gcn/data/corpus/' + dataset + '_vocab.txt', 'r')
    words = f.readlines()
    f.close()
    ...
    word_embeddings_str = '\n'.join(word_vectors)
    f = open('./text_gcn/data/' + dataset + '_word_vectors.txt', 'w')
    ...
    doc_embeddings_str = '\n'.join(doc_vectors)
    f = open('./text_gcn/data/' + dataset + '_doc_vectors.txt', 'w')
    f.write(doc_embeddings_str)
    f.close()

    return round(test_acc, 6)
